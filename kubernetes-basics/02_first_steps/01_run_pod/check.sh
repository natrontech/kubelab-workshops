...
kubectl get pods -n app1 --field-selector=status.phase=Running --output=json | jq -e '.items[] | select(.metadata.name=="simple-pod" and .spec.containers[].image=="nginx")' > /dev/null
